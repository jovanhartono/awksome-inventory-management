      ...
      <section>
        ...
        <div className="pt-3">
          <form className={"space-y-3"} onSubmit={submitForm}>
            ...
            {productDetails.map(
              (productDetail: ProductDetail, index: number) => {
                return (
            ...
            <div>
              <button type="submit" className={"mt-3 button-submit"}>
                Add Order
              </button>
            </div>
          </form>
...
export const getStaticProps: GetStaticProps = async () => {
  ...
  const mappedProducts: Array<ProductDropdown> = products.map(
    ({ id, name }) => ({ id, name })
  );
